...
function renderTables(){     
    ...
    availableTables.forEach(table => {     
        ...
        tableDiv.innerHTML = `<img src="reserva.png" alt="Mesa ${table.number}"><div class="table-name">Mesa ${table.number}</div>`;       
...
occupiedTables.forEach(table =>{
    const tableDiv = document.createElement('div');
    tableDiv.className ='table';
    tableDiv.innerHTML = `<img src="reserva.png" alt="Mesa ${table.
    number}"><div class="table-name">Mesa ${table.number}</div>`;
...
function reserveTable() {
    ...
    if (!customerName || isNaN(tableNumber) || tableNumber < 1 || tableNumber > availableTables.length || !reservationDate || !reservationTime) {
        alert('Por favor, ingrese un nombre válido, un número de mesa válido y seleccione bien la fecha y hora.');
        return;
    }

    if (isTimeSlotAvailable(tableNumber, dateTime)) {
        occupiedTables.push({ number: tableNumber, customer: customerName, dateTime: dateTime });
        renderTables();
        clearForm();
...
function reserveTableByNumber(tableNumber) {
    ...
    if (!reservationDate || !reservationTime) {
        alert('Por favor ingrese una fecha y hora para la reserva.');
        return;
    }
    ...
    if (isTimeSlotAvailable(tableNumber, dateTime)) {
        ...
        alert("La mesa no está disponible en ese horario.\nDebe haber mínimo 3 horas de diferencia para prestar la misma mesa.");
    }
}
...
function isTimeSlotAvailable(tableNumber, newDateTime) {
    ...
    const minDifference = 3 * 60 * 60 * 1000; // 3 horas
